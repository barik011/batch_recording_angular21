...
export class BatchServices {
  ...
  getAllBatchService():Observable<IAPIResponse>{
    return this.http.get<IAPIResponse>(environment.API_URL + Controllers.BATCHES);
  }

  addBatchService(obj:BatchModel):Observable<IAPIResponse>{
    return this.http.post<IAPIResponse>(environment.API_URL + Controllers.BATCHES,obj)
  }
  getBatchByIdService(item:BatchModel){
    return this.http.get<IAPIResponse>(environment.API_URL + Controllers.BATCHES+'/'+item.batchId);
  }
  updateBatchService(obj:BatchModel):Observable<IAPIResponse>{
    return this.http.put<IAPIResponse>(environment.API_URL + Controllers.BATCHES+'/'+obj.batchId,obj)
  }
  deleteBatchService(id:number):Observable<IAPIResponse>{
    return this.http.delete<IAPIResponse>(environment.API_URL + Controllers.BATCHES+'/'+id)
  }
}
